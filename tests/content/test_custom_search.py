...
class ResultsApiTests(BaseCustomSearchApiTests):
    ...
    def check_api_counts(self, query, expected_count):
        endpoint_url = reverse("content-custom-filter-items")
...
class CountsApiTests(BaseCustomSearchApiTests):
    ...
    def check_api_counts(self, query, expected_count):
        endpoint_url = reverse("content-custom-filter-count")
        r = self.api_client.post(endpoint_url, {"query": query, "preview": False}, format="json")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.data["count"], expected_count)
    ...
    def check_api_group_preview_counts(self, query, expected_count):
        ...
        if not groups:
            return
        group = groups[0]
        endpoint_url = reverse("content-custom-filter-group-count")
